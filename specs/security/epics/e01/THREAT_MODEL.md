...
- **Mitigation (planned):** CSP per `SECURITY_PLAN_LATEST` — `script-src 'self' 'unsafe-inline' https://va.vercel-scripts.com`; `style-src 'self' 'unsafe-inline' https://fonts.googleapis.com`; `font-src 'self' https://fonts.gstatic.com`; `img-src 'self' https://lh3.googleusercontent.com data: blob:`; `connect-src 'self' https://va.vercel-scripts.com`; `frame-ancestors 'none'`. No `'unsafe-eval'`.
- **`'unsafe-inline'` for scripts is a documented deviation (e08 verification finding):** Next.js injects inline bootstrap scripts for hydration; a strict `script-src 'self'` blocks them and the app never hydrates (proven by E2E — React error #412). The trade-off is accepted because the app has **zero injection sinks** (no `dangerouslySetInnerHTML` anywhere — enforced by lint + audit + repo grep) and React escapes all rendering; CSP still blocks arbitrary external hosts and `'unsafe-eval'`. A nonce-based CSP is the future hardening path (documented in CONVENTIONS/ADR context) but is not warranted for a zero-sink demo.
- **Task mapping:** e01s02-1 (`security: high`). E2E **N9** asserts the headers on every route.
